...
@Component({
  selector: 'app-all-tax-master-audit-trail',
  templateUrl: './all-tax-master-audit-trail.component.html',
  styleUrls: ['./all-tax-master-audit-trail.component.scss']
})
export class AllTaxMasterAuditTrailComponent implements OnInit{
  @ViewChild('htmlData') htmlData!: ElementRef;

  constructor(public adminService:AdminService,
    private taxMasterService: TaxMasterService,
    public messageService:MessageService,
    public dialogRef: MatDialogRef<AllTaxMasterAuditTrailComponent>,
  ...
  onSearch(){
    this.isLoading=true;
    this.taxMasterService.onAllRoleAuditTrail(this.userData.tableData.uc0001).subscribe((data: any) => {
      console.log(data);
      this.data=data.data;
  ...
  fileWidth:any;
     ...
     onDownloadPDF(){
      ...
      if(this.dataLength==1){
       ...
       openPDFByTwo(HEADER,DATA,DATA1,50,22,'tax')
      }else if(this.dataLength==3){
        let DATA: any = document.getElementById('Location1');
       let DATA1: any = document.getElementById('Location2');
       let DATA2: any = document.getElementById('Location3');
       let HEADER: any = document.getElementById('header');
        openPDFByThree(HEADER,DATA,DATA1,DATA2,50,22,'tax');
      }
      ...
      let HEADER: any = document.getElementById('header');
      openPDFByFour(HEADER,DATA,DATA1,DATA2,DATA3,50,22,'tax');
    }
    else {
      let DATA: any = document.getElementById('Location1');
      let DATA1: any = document.getElementById('Location2');
      ...
      let HEADER: any = document.getElementById('header');
      openPDFByFive(HEADER,DATA,DATA1,DATA2,DATA3,DATA4,50,22,'tax');
    }
     }

     public openPDFByOne(): void {
      let DATA: any = document.getElementById('htmlData');
      downloadCanvasArea(DATA,'tax')
     }
