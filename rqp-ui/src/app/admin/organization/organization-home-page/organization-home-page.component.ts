...
@Component({
  ...
  styleUrls: ['./organization-home-page.component.scss']
})
export class OrganizationHomePageComponent implements OnInit, AfterViewInit {
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.organizationService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
  ...
  onActiveSelectRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(OrganizationCreateUpdateComponent,{
          minWidth:"80%",
