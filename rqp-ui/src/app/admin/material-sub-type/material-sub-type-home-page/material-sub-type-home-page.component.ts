...
import { MaterialSubTypeCreateUpdateComponent } from '../material-sub-type-create-update/material-sub-type-create-update.component';
import { exportData } from 'bk-export'
import { AllSubMaterialAuditTrailComponent } from '../all-sub-material-audit-trail/all-sub-material-audit-trail.component';
import { ActiveSubMaterialAuditTrailComponent } from '../active-sub-material-audit-trail/active-sub-material-audit-trail.component';

@Component({
  selector: 'app-material-sub-type-home-page',
  templateUrl: './material-sub-type-home-page.component.html',
  styleUrls: ['./material-sub-type-home-page.component.scss']
})
export class MaterialSubTypeHomePageComponent implements OnInit, AfterViewInit {
  ...
  constructor(private _liveAnnouncer: LiveAnnouncer,
    ...
    public materialSubTypeService:MaterialSubTypeService) {}
  ...
  onLoadAllRoleMaster(){
    ...
    this.pageIndex = 0;
    this.materialSubTypeService.getAllDepartment(this.size, this.pageIndex).subscribe((data: any) => {
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.materialSubTypeService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
  ...
  OnLoadActiveRoleMaster(){
    ...
    this.dataSource = null;
    this.pageIndex = 0;
    this.materialSubTypeService.getActiveDepartment(this.size, this.pageIndex).subscribe((data: any) => {
      if(data.errorInfo !=null){
        this.dialog.open(MessageDialogComponent, {
          data: { 'message': data.errorInfo.message, 'heading': "Error Information" }
        });
  ...
  activeUserDownloadTxt(){
   ...
   for(var i=0, len=excelData.length; i<len; i++){
     arrExcel.push({
      "Id":excelData[i].id,
      "Material-Sub Type Code":excelData[i].uc0001,
      "Material-Sub Type Name":excelData[i].ff0001,
   ...
   exportData(arrExcel,'mst','mst','txt')
   }
   ...
   exportData(arrExcel,'mst','mst','csv')
  }
  activeUserDownloadPdf(){
    let header: string[] = ['Id','Material sub Type Code', 'Material sub Type Name', 'Business Unit Code', 'Status','Version','Created Date','CreatedBy'];
   this.totalRow=0;
   var img = new Image();
   ...
   doc.setFontSize(14); 
   doc.text("Material Sub Type", 86, 30);
  ...
  exportData(arrExcel,'mst','mst','excel')
  }
  ...
  onAllSelectAuditRow(){
    if(this.selectedAllRow.length==0){
      ...
      const dialogRef=this.dialog.open(AllSubMaterialAuditTrailComponent,{
          minWidth:"80%",
  ...
  onActiveSelectAuditRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(ActiveSubMaterialAuditTrailComponent,{
          minWidth:"80%",
  ...
  onActiveSelectRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(MaterialSubTypeCreateUpdateComponent,{
          minWidth:"80%",
