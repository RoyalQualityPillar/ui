...
import {DesignationCreateUpdateComponent} from '../designation-create-update/designation-create-update.component';
import { exportData } from 'bk-export';
import { ActiveDesigantionAuditTrailComponent } from '../active-desigantion-audit-trail/active-desigantion-audit-trail.component';
import { AllDesigantionAuditTrailComponent } from '../all-desigantion-audit-trail/all-desigantion-audit-trail.component';


@Component({
  selector: 'app-designation-home-page',
  templateUrl: './designation-home-page.component.html',
  styleUrls: ['./designation-home-page.component.scss']
})
export class DesignationHomePageComponent implements OnInit, AfterViewInit {
  ...
  onLoadAllRoleMaster(){
    ...
    this.pageIndex = 0;
    this.designationService.getAllDepartment(this.size, this.pageIndex).subscribe((data: any) => {
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.designationService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
  ...
  OnLoadActiveRoleMaster(){
    ...
    this.dataSource = null;
    this.pageIndex = 0;
    this.designationService.getActiveDepartment(this.size, this.pageIndex).subscribe((data: any) => {
      if(data.errorInfo !=null){
        this.dialog.open(MessageDialogComponent, {
          data: { 'message': data.errorInfo.message, 'heading': "Error Information" }
        });
  ...
  activeUserDownloadTxt(){
   ...
   for(var i=0, len=excelData.length; i<len; i++){
     arrExcel.push({
      "Id":excelData[i].id,
      "Designation Code":excelData[i].uc0001,
      "Designation Name":excelData[i].ff0001,
  ...
  exportData(arrExcel,'desigination','desigination','txt')
   }
  ...
  exportData(arrExcel,'desigination','desigination','csv')
  }
  activeUserDownloadPdf(){
    let header: string[] = ['Id', 'Designation Code', 'Designation Name','Business Unit Code', 'Status','Version','Created Date','CreatedBy'];
   this.totalRow=0;
   var img = new Image();
   ...
   doc.setFontSize(14); 
   doc.text("Designation Master", 86, 30);
  ...
  exportData(arrExcel,'desigination','desigination','excel')
  }
  copyData() {
    var dataArray = "";
    let tableData:any;
   ...
   downloadPdf() {
    ...
    doc.rect(5, 24, 200, 8, "F");
    doc.setFontSize(14); 
    doc.text("Designation Master", 88, 30);
    doc.addImage(img, 'gif', 170, 5, 30, 15);
    autoTable(doc, {
      head: col,
      body: rows,
 ...
 exportData(arrExcel,'desigination','desigination','excel')
  ...
  onActiveAuditSelectRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(ActiveDesigantionAuditTrailComponent,{
          minWidth:"80%",
  ...
  onAllAuditSelectRow(){
    if(this.selectedAllRow.length==0){
      ...
      const dialogRef=this.dialog.open(AllDesigantionAuditTrailComponent,{
          minWidth:"80%",
  ...
  onActiveSelectRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(DesignationCreateUpdateComponent,{
          minWidth:"80%",
          data:{tableData:this.selectedRow,type:'Update'}
        })
        dialogRef.afterClosed().subscribe(result => {
