...
@Component({
  selector: 'app-all-but-audit-trail',
  templateUrl: './all-but-audit-trail.component.html',
  styleUrls: ['./all-but-audit-trail.component.scss']
})
export class AllButAuditTrailComponent implements OnInit{
  @ViewChild('htmlData') htmlData!: ElementRef;

  constructor(public adminService:AdminService,
    private businessUnitTypeService: BusinessUnitTypeService,
    public messageService:MessageService,
    public dialogRef: MatDialogRef<AllButAuditTrailComponent>,
  ...
  onSearch(){
    this.isLoading=true;
    this.businessUnitTypeService.onAllRoleAuditTrail(this.userData.tableData.uc0001).subscribe((data: any) => {
      console.log(data);
      this.data=data.data;
  ...
  fileWidth:any;
     ...
     onDownloadPDF(){
      ...
      if(this.dataLength==1){
       ...
       openPDFByTwo(HEADER,DATA,DATA1,50,22,'but')
      }else if(this.dataLength==3){
        let DATA: any = document.getElementById('Location1');
       let DATA1: any = document.getElementById('Location2');
       let DATA2: any = document.getElementById('Location3');
       let HEADER: any = document.getElementById('header');
        openPDFByThree(HEADER,DATA,DATA1,DATA2,50,22,'but');
      }
      ...
      let HEADER: any = document.getElementById('header');
      openPDFByFour(HEADER,DATA,DATA1,DATA2,DATA3,50,22,'but');
    }
    else {
      let DATA: any = document.getElementById('Location1');
      let DATA1: any = document.getElementById('Location2');
      ...
      let HEADER: any = document.getElementById('header');
      openPDFByFive(HEADER,DATA,DATA1,DATA2,DATA3,DATA4,50,22,'but');
    }
     }
