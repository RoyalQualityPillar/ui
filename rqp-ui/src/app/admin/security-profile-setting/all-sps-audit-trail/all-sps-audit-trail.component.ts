...
@Component({
  selector: 'app-all-sps-audit-trail',
  templateUrl: './all-sps-audit-trail.component.html',
  styleUrls: ['./all-sps-audit-trail.component.scss']
})
export class AllSpsAuditTrailComponent implements OnInit{
  @ViewChild('htmlData') htmlData!: ElementRef;

  constructor(public adminService:AdminService,
    private securityProfileService: SecurityProfileService,
    public messageService:MessageService,
    public dialogRef: MatDialogRef<AllSpsAuditTrailComponent>,
  ...
  onSearch(){
    this.isLoading=true;
    this.securityProfileService.onAllRoleAuditTrail(this.userData.tableData.uc0001).subscribe((data: any) => {
      console.log(data);
      this.data=data.data;
  ...
  fileWidth:any;
     ...
     onDownloadPDF(){
      ...
      if(this.dataLength==1){
       ...
       openPDFByTwo(HEADER,DATA,DATA1,50,22,'sps')
      }else if(this.dataLength==3){
        let DATA: any = document.getElementById('Location1');
       let DATA1: any = document.getElementById('Location2');
       let DATA2: any = document.getElementById('Location3');
       let HEADER: any = document.getElementById('header');
        openPDFByThree(HEADER,DATA,DATA1,DATA2,50,22,'sps');
      }
      ...
      let HEADER: any = document.getElementById('header');
      openPDFByFour(HEADER,DATA,DATA1,DATA2,DATA3,50,22,'sps');
    }
    else {
      let DATA: any = document.getElementById('Location1');
      let DATA1: any = document.getElementById('Location2');
      ...
      let HEADER: any = document.getElementById('header');
      openPDFByFive(HEADER,DATA,DATA1,DATA2,DATA3,DATA4,50,22,'sps');
    }
     }
