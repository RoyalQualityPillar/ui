...
import { CreateUpdatePriceTypeComponent } from '../create-update-price-type/create-update-price-type.component';
import { AllPriceTypeAtComponent } from '../all-price-type-at/all-price-type-at.component';
import { ActivePriceTypeAtComponent } from '../active-price-type-at/active-price-type-at.component';


@Component({
  selector: 'app-price-type-home-page',
  templateUrl: './price-type-home-page.component.html',
  styleUrls: ['./price-type-home-page.component.scss']
})
export class PriceTypeHomePageComponent implements OnInit,AfterViewInit{
  ...
  tableData: MatTableDataSource<any>;
   constructor(private router:Router,
    private paymentTermService:PriceTypeService,
  ...
  selectedRow=[];
  onOpenRolePOPUP(){
    const dialogRef=this.dialog.open(CreateUpdatePriceTypeComponent,{
      minWidth:"80%",
  ...
  onAllSelectAuditRow(){
    if(this.selectedAllRow.length==0){
      ...
      const dialogRef=this.dialog.open(AllPriceTypeAtComponent,{
          minWidth:"80%",
  ...
  copyData() {
    ...
    onActiveSelectAuditRow(){
      if(this.selectedRow.length==0){
        ...
        const dialogRef=this.dialog.open(ActivePriceTypeAtComponent,{
            minWidth:"80%",
            data:{tableData:this.selectedRow,type:'Update'}
          })
          dialogRef.afterClosed().subscribe(result => {
    ...
    downloadTxt(){
      ...
      for(var i=0, len=excelData.length; i<len; i++){
        arrExcel.push({
          "Price Type Code":excelData[i].uc0001,
          "Price Type Name":excelData[i].ff0001,
      ...
      exportData(arrExcel,'pricetype','pricetype','txt')
}
downloadCsvFile() {
  let excelData=JSON.parse(JSON.stringify(this.tableData.filteredData))
  let arrExcel=[];
  ...
  exportData(arrExcel,'pricetype','pricetype','csv')
}


totalRow:any;
downloadPdf() {
 let header: string[] = ['Price Type Code', 'Payment Type Name', 'Business Unit Code', 'Status','Version'];
 this.totalRow=0;
 var img = new Image();
...
exportData(arrExcel,'pricetype','pricetype','excel')
}
...
activeUserDownloadCsvFile(){
 ...
 activeUserDownloadPdf(){
  let header: string[] = ['Price Type Code', 'Price Type Name', 'Business Unit Code', 'Status','Version'];
 this.totalRow=0;
 var img = new Image();
 img.src = 'assets/logo1.png'
 let doc = new jsPDF('p', 'mm', 'A4')
