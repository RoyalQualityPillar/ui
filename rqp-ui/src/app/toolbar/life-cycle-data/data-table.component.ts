...
export class DataTableComponent implements OnInit, AfterViewInit {
  ...
  redirect(data: any) {
    ...
    //need to add PQT and fair object for update
    let isfindSuccessDQ = 0;
    let dqObj = { ff0001: "Sale Quatetion", lcnum: "RQP1DQWHLC0002", lcrole: "DQ-Update", stage: 1, userId: this.selectedRow.userid }
    data.forEach((ele) => {
    ...
    let isfindSuccessPQT = 0;
    let pqtObj = { ff0001: "Sale Quatetion", lcnum: "RQP1DQWHLC0002", lcrole: "PQT-Update", stage: 1, userId: this.selectedRow.userid }
    data.forEach((ele) => {
      if (ele.lcrole == 'PQT-Initator' || ele.lcrole == 'PQT-Reviewer') {
        ++isfindSuccessPQT;
      }
    })
    if (isfindSuccessPQT > 0) {
      data.push(pqtObj)
    }
    ...
    let isfindSuccessPsi = 0;
    let psiObj = { ff0001: "Sale Quatetion", lcnum: "RQP1DQWHLC0002", lcrole: "PSI-Update", stage: 1, userId: this.selectedRow.userid }
    data.forEach((ele) => {
      if (ele.lcrole == 'PSI-Initator' || ele.lcrole == 'PSI-Reviewer') {
        ++isfindSuccessPsi;
      }
    })
    if (isfindSuccessPsi > 0) {
      data.push(psiObj)
    }
