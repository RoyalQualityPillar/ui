...
import { CreateUpdateSaleProductMasterComponent } from '../create-update-sale-product-master/create-update-sale-product-master.component';
import { AllSaleProductMasterAtComponent } from '../all-sale-product-master-at/all-sale-product-master-at.component';
import { ActiveSaleProductMasterAtComponent } from '../active-sale-product-master-at/active-sale-product-master-at.component';

@Component({
  selector: 'app-sale-product-master-home-page',
  templateUrl: './sale-product-master-home-page.component.html',
  styleUrls: ['./sale-product-master-home-page.component.scss']
})
export class SaleProductMasterHomePageComponent implements OnInit,AfterViewInit{
  ...
  tableData: MatTableDataSource<any>;
   constructor(private router:Router,private saleProductMasterService:SaleProductMasterService,
  ...
  selectedRow=[];
  onOpenRolePOPUP(){
    const dialogRef=this.dialog.open(CreateUpdateSaleProductMasterComponent,{
      minWidth:"80%",
  ...
  onAllSelectAuditRow(){
    if(this.selectedAllRow.length==0){
      ...
      const dialogRef=this.dialog.open(AllSaleProductMasterAtComponent,{
          minWidth:"80%",
  ...
  onLoadAllSaleProductMaster(){
    ...
    this.saleProductMasterService.getAllSaleProduct(this.size,this.pageIndex).subscribe((data:any)=>{
  ...
  onLoadActiveSaleProductMaster(){
    ...
    this.pageIndex = 0;
    this.saleProductMasterService.getActiveSaleProduct(this.size, this.pageIndex).subscribe((data: any) => {
      if(data.errorInfo !=null){
        this.dialog.open(MessageDialogComponent, {
          data: { 'message': data.errorInfo.message, 'heading': "Error Information" }
        });
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.saleProductMasterService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
  ...
  copyData() {
    ...
    onActiveSelectAuditRow(){
      if(this.selectedRow.length==0){
        ...
        const dialogRef=this.dialog.open(ActiveSaleProductMasterAtComponent,{
            minWidth:"80%",
            data:{tableData:this.selectedRow,type:'Update'}
          })
          dialogRef.afterClosed().subscribe(result => {
