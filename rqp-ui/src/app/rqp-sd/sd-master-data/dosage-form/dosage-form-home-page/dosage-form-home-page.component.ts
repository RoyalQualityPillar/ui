...
import { CreateUpdateDosageFormComponent } from '../create-update-dosage-form/create-update-dosage-form.component';
import { AllDosageFormAtComponent } from '../all-dosage-form-at/all-dosage-form-at.component';
import { ActiveDosageFormAtComponent } from '../active-dosage-form-at/active-dosage-form-at.component';



@Component({
  selector: 'app-dosage-form-home-page',
  templateUrl: './dosage-form-home-page.component.html',
  styleUrls: ['./dosage-form-home-page.component.scss']
})
export class DosageFormHomePageComponent implements OnInit,AfterViewInit{
  ...
  tableData: MatTableDataSource<any>;
   constructor(private router:Router,
    private paymentTermService:DosageFormService,
  ...
  selectedRow=[];
  onOpenRolePOPUP(){
    const dialogRef=this.dialog.open(CreateUpdateDosageFormComponent,{
      minWidth:"80%",
  ...
  onAllSelectAuditRow(){
    if(this.selectedAllRow.length==0){
      ...
      const dialogRef=this.dialog.open(AllDosageFormAtComponent,{
          minWidth:"80%",
  ...
  copyData() {
    ...
    onActiveSelectAuditRow(){
      if(this.selectedRow.length==0){
        ...
        const dialogRef=this.dialog.open(ActiveDosageFormAtComponent,{
            minWidth:"80%",
            data:{tableData:this.selectedRow,type:'Update'}
          })
          dialogRef.afterClosed().subscribe(result => {
    ...
    downloadTxt(){
      ...
      for(var i=0, len=excelData.length; i<len; i++){
        arrExcel.push({
          "Dosage Form Code":excelData[i].uc0001,
          "Dosage Form Name":excelData[i].ff0001,
      ...
      exportData(arrExcel,'dosageForm','dosageForm','txt')
}
downloadCsvFile() {
  let excelData=JSON.parse(JSON.stringify(this.tableData.filteredData))
  let arrExcel=[];
  ...
  exportData(arrExcel,'dosageForm','dosageForm','csv')
}


totalRow:any;
downloadPdf() {
 let header: string[] = ['Dosage Form Code', 'Dosage Form Name', 'Business Unit Code', 'Status','Version'];
 this.totalRow=0;
 var img = new Image();
...
exportData(arrExcel,'dosageForm','dosageForm','excel')
}
...
activeUserDownloadExcel1(){
 ...
 activeUserDownloadExcel(){ 
  ...
  exportData(arrExcel,'dosageForm','dosageFrom','excel')
}
