...
@Component({
  ...
  templateUrl: './standard-reason-registartion-home-page.component.html',
  styleUrls: ['./standard-reason-registartion-home-page.component.scss']
})
export class StandardReasonRegistartionHomePageComponent implements OnInit, AfterViewInit {
  ...
  constructor(private _liveAnnouncer: LiveAnnouncer,
    ...
    public standardReasonRegistartionService:StandardReasonRegistartionService) {}
  ...
  onLoadAllRoleMaster(){
    ...
    this.pageIndex = 0;
    this.standardReasonRegistartionService.getAllDepartment(this.size, this.pageIndex).subscribe((data: any) => {
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.standardReasonRegistartionService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
  ...
  OnLoadActiveRoleMaster(){
    ...
    this.dataSource = null;
    this.pageIndex = 0;
    this.standardReasonRegistartionService.getActiveDepartment(this.size, this.pageIndex).subscribe((data: any) => {
      if(data.errorInfo !=null){
        this.dialog.open(MessageDialogComponent, {
          data: { 'message': data.errorInfo.message, 'heading': "Error Information" }
        });
  ...
  activeUserDownloadPdf(){
    let header: string[] = ['Id','Standard Reason Code', 'Business Unit Code', 'Module Code','Standard Reason', 'Status','Version','Created Date','CreatedBy'];
   this.totalRow=0;
   var img = new Image();
   ...
   doc.setFontSize(14); 
   doc.text("Standard Reason", 86, 30);
  ...
  copyData() {
   ...
   downloadPdf() {
    ...
    doc.rect(5, 24, 200, 8, "F");
    doc.setFontSize(14); 
    doc.text("Standard Reason", 88, 30);
    doc.addImage(img, 'gif', 170, 5, 30, 15);
    autoTable(doc, {
      head: col,
      body: rows,
  ...
  onActiveSelectRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(StandardReasonRegistartionCreateUpdateComponent,{
          minWidth:"80%",
