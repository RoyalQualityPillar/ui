...
@Component({
  ...
  styleUrls: ['./tax-master-home-page.component.scss']
})
export class TaxMasterHomePageComponent implements OnInit, AfterViewInit {
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.taxMasterService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
 ...
 exportData(arrExcel,'tax','tax','excel')
  ...
  onActiveSelectRow(){
    if(this.selectedRow.length==0){
      ...
      const dialogRef=this.dialog.open(TaxMasterCreateUpdateComponent,{
          minWidth:"80%",
