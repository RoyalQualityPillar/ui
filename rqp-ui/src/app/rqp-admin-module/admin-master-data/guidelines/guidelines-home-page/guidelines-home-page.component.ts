...
import { GuidelinesCreateUpdateComponent } from '../guidelines-create-update/guidelines-create-update.component';
import { GlobalConstants } from 'src/app/common/global-constants';
import { ToolbarService } from 'src/app/service/toolbar.service';


@Component({
  selector: 'app-guidelines-home-page',
  templateUrl: './guidelines-home-page.component.html',
  styleUrls: ['./guidelines-home-page.component.scss']
})
export class GuidelinesHomePageComponent implements OnInit, AfterViewInit {
  ...
  onLoadAllRoleMaster(){
    ...
    this.pageIndex = 0;
    this.guidelinesService.getAllDepartment(this.size, this.pageIndex).subscribe((data: any) => {
  ...
  applyFilterByColumn(){
    ...
    this.isLoading=true;
    this.guidelinesService.getUserProfileFilterData(filetrDataBody).subscribe((data: any) => {
      console.log(data)
      if(data.data){
  ...
  OnLoadActiveRoleMaster(){
    ...
    this.dataSource = null;
    this.pageIndex = 0;
    this.guidelinesService.getActiveDepartment(this.size, this.pageIndex).subscribe((data: any) => {
  ...
  activeUserDownloadPdf(){
   ...
   doc.setFontSize(14); 
   doc.text("Business Unit Type", 86, 30);
  ...
  copyData() {
   ...
   downloadPdf() {
    ...
    doc.rect(5, 24, 200, 8, "F");
    doc.setFontSize(14); 
    doc.text("Business Unit Type", 88, 30);
    doc.addImage(img, 'gif', 170, 5, 30, 15);
    autoTable(doc, {
      head: col,
      body: rows,
  ...
  onActiveSelectRow(){
    ...
    //  })
    //   }else{
    //   const dialogRef=this.dialog.open(BusinessUnitTypeCreateUpdateComponent,{
    //       minWidth:"80%",
  ...
  onOpenRolePOPUP(){
    console.log('working')
    const dialogRef=this.dialog.open(GuidelinesCreateUpdateComponent,{
      minWidth:"80%",
